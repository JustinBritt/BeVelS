namespace BeVelS.Graphics.Buffers.Factories.VertexConstants
{
    ...
    internal sealed class MeshVertexConstantsBufferFactory : IMeshVertexConstantsBufferFactory
    {
        private ILog Log => LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public MeshVertexConstantsBufferFactory()
        {
        }

        public unsafe IConstantsBuffer<MeshVertexConstants> Create(
            IBufferDescriptionFactory bufferDescriptionFactory,
            GraphicsDevice graphicsDevice)
        {
            IConstantsBuffer<MeshVertexConstants> constantsBuffer = null;

            try
            {
                constantsBuffer = new ConstantsBuffer<MeshVertexConstants>(
                    graphicsDevice.ResourceFactory.CreateBuffer(
                        bufferDescriptionFactory.Create(
                            Unsafe.SizeOf<MeshVertexConstants>().RoundUpToPowerOf2(
                                4),
