namespace BeVelS.Graphics.Buffers.Factories.VertexConstants
{
    ...
    internal sealed class GlyphVertexConstantsBufferFactory : IGlyphVertexConstantsBufferFactory
    {
        private ILog Log => LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public GlyphVertexConstantsBufferFactory()
        {
        }

        public unsafe IConstantsBuffer<GlyphVertexConstants> Create(
            IBufferDescriptionFactory bufferDescriptionFactory,
            GraphicsDevice graphicsDevice)
        {
            IConstantsBuffer<GlyphVertexConstants> constantsBuffer = null;

            try
            {
                constantsBuffer = new ConstantsBuffer<GlyphVertexConstants>(
                    graphicsDevice.ResourceFactory.CreateBuffer(
                        bufferDescriptionFactory.Create(
                            Unsafe.SizeOf<GlyphVertexConstants>().RoundUpToPowerOf2(
                                4),
