namespace BeVelS.Graphics.Buffers.Factories.VertexConstants
{
    ...
    internal sealed class HTMLVertexConstantsBufferFactory : IHTMLVertexConstantsBufferFactory
    {
        private ILog Log => LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public HTMLVertexConstantsBufferFactory()
        {
        }

        public unsafe IConstantsBuffer<HTMLVertexConstants> Create(
            IBufferDescriptionFactory bufferDescriptionFactory,
            GraphicsDevice graphicsDevice)
        {
            IConstantsBuffer<HTMLVertexConstants> constantsBuffer = null;

            try
            {
                constantsBuffer = new ConstantsBuffer<HTMLVertexConstants>(
                    graphicsDevice.ResourceFactory.CreateBuffer(
                        bufferDescriptionFactory.Create(
                            Unsafe.SizeOf<HTMLVertexConstants>().RoundUpToPowerOf2(
                                4),
