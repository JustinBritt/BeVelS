namespace BeVelS.Graphics.Buffers.Factories.VertexConstants
{
    ...
    internal sealed class UILineVertexConstantsBufferFactory : IUILineVertexConstantsBufferFactory
    {
        private ILog Log => LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public UILineVertexConstantsBufferFactory()
        {
        }

        public unsafe IConstantsBuffer<UILineVertexConstants> Create(
            IBufferDescriptionFactory bufferDescriptionFactory,
            GraphicsDevice graphicsDevice)
        {
            IConstantsBuffer<UILineVertexConstants> constantsBuffer = null;

            try
            {
                constantsBuffer = new ConstantsBuffer<UILineVertexConstants>(
                    graphicsDevice.ResourceFactory.CreateBuffer(
                        bufferDescriptionFactory.Create(
                            Unsafe.SizeOf<UILineVertexConstants>().RoundUpToPowerOf2(
                                4),
