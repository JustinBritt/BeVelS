namespace BeVelS.Graphics.Fonts.Classes
{
    ...
    [BepuPhysicsLicensedCode(
        ...
        source: "https://github.com/bepu/bepuphysics2/blob/master/DemoContentBuilder/FontBuilder.cs",
        BepuPhysicsLicensedCodeAttribute.FormattingChanges)]
    public static class FontBuilder
    {
        private static Dictionary<CharacterPair, int> ComputeKerningTable(
            Face face,
            string characterSet)
        {
            ...
            for (int i = 0; i < characterSet.Length; ++i)
            {
                ...
                {
                    FTVector26Dot6 kerning = face.GetKerning(
                        glyphIndex,
                        face.GetCharIndex(
                            characterSet[i]),
                        KerningMode.Default);
        ...
        public unsafe static IFontContent Build(
            ...
            Stream fontDataStream)
        {
            ...
            using (Library library = new Library())
            {
                using (Face face = new Face(library, faceBytes, 0))
                {
                    ...
                    ITextureContent rasterizedAlphas = textureContentFactory.Create(
                        vector2Factory,
                        AtlasWidth,
                        packer.Height,
                        1,
                    ...
                    ITextureContent preciseAtlas = textureContentFactory.Create(
                        vector2Factory,
                        AtlasWidth,
                        packer.Height,
                        MipLevels,
                    ...
                    ITextureContent atlas = textureContentFactory.Create(
                        vector2Factory,
                        AtlasWidth,
                        packer.Height,
                        MipLevels,
                    ...
                    //Bitmap bitmap = new Bitmap(atlas.Width >> savedMip, atlas.Height >> savedMip);
                    //BitmapData bitmapData = bitmap.LockBits(new Rectangle(new Point(), new Size(bitmap.Width, bitmap.Height)),
                    //    ImageLockMode.WriteOnly, PixelFormat.Format32bppRgb);

                    //byte * scan0 = (byte *)bitmapData.Scan0;
                    //byte * sourceStart = atlasData + atlas.GetMipStartIndex(savedMip);
                    //for (int rowIndex = 0; rowIndex < bitmapData.Height; ++rowIndex)
                    //{
                    //    int * row = (int *)(scan0 + bitmapData.Stride * rowIndex);
                    //    byte * sourceRow = sourceStart + atlas.GetRowOffsetFromMipStart(savedMip, rowIndex);
                    ...
                    Dictionary<CharacterPair, int> kerning = ComputeKerningTable(
                        face,
                        characterSet);
                    ...
                    {
                        characters.Add(
                            sortedCharacterSet[i],
                            sortedCharacterData[i]);
                    }

                    return fontContentFactory.Create(
                        atlas,
                        face.FamilyName,
                        1f / FontSizeInPixels,
                        characters,
