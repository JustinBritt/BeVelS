namespace BeVelS.Physics.Constraints.Classes
{
    ...
    [BepuPhysicsLicensedCode(
        ...
        "Use ColorUtilities")]
    public class BoundingBoxLineExtractor : IBoundingBoxLineExtractor
    {
        ...
        public static void WriteBoundsLines(
            Vector3 min,
            Vector3 max,
            uint packedColor,
            uint packedBackgroundColor,
            ref LineInstance targetLines)
        {
            ...
            Unsafe.Add(ref targetLines, 0) = new LineInstance(
                min,
                v001,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 1) = new LineInstance(
                min,
                v010,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 2) = new LineInstance(
                min,
                v100,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 3) = new LineInstance(
                v001,
                v011,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 4) = new LineInstance(
                v001,
                v101,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 5) = new LineInstance(
                v010,
                v011,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 6) = new LineInstance(
                v010,
                v110,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 7) = new LineInstance(
                v011,
                max,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 8) = new LineInstance(
                v100,
                v101,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 9) = new LineInstance(
                v100,
                v110,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 10) = new LineInstance(
                v101,
                max,
                packedColor,
                packedBackgroundColor);
            
            Unsafe.Add(ref targetLines, 11) = new LineInstance(
                v110,
                max,
                packedColor,
                packedBackgroundColor);
        ...
        internal unsafe void ExecuteJob(
            ...
            ThreadJob job)
        {
            ...
            var packedBackgroundColor = ColorUtilities.PackColor(
                backgroundColor);
            ...
            for (int i = 0; i < job.LeafCount; ++i)
            {
                ...
                if (job.CoversActiveCollidables)
                    simulation.BroadPhase.GetActiveBoundsPointers(
                        broadPhaseIndex,
                        out min,
                        out max);
                else
                    simulation.BroadPhase.GetStaticBoundsPointers(
                        broadPhaseIndex,
                        out min,
                        out max);
                ...
                WriteBoundsLines(
                    ...
                    *max,
                    packedColor,
                    packedBackgroundColor,
                    ref lines[outputStartIndex]);
            }
        }

        void CreateJobsForTree(
            in Tree tree,
            bool active,
            int simulationIndex,
            ref QuickList<ThreadJob> jobs,
            ref int newLinesCount,
            ref int nextStart,
            BufferPool pool)
        {
        ...
        internal unsafe void CreateJobs(
            Simulation simulation,
            int simulationIndex,
            ...
            BufferPool pool)
        {
            ...
            lines.EnsureCapacity(
                lines.Count + 12 * (simulation.BroadPhase.ActiveTree.LeafCount + simulation.BroadPhase.StaticTree.LeafCount),
                pool);
            ...
            CreateJobsForTree(
                simulation.BroadPhase.ActiveTree,
                true,
                simulationIndex,
                ref jobs,
                ref newLinesCount,
            ...
            CreateJobsForTree(
                simulation.BroadPhase.StaticTree,
                false,
                simulationIndex,
                ref jobs,
                ref newLinesCount,
